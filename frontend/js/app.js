...
const posters = {
  ...
  "Aventura Pixel": "aventura-pixel.jpeg",
  "Planeta Aurora": "planeta-aurora.jpeg",
  "Cartas de Invierno": "cartas-de-invierno.jpeg",
  "El Tesoro Perdido": "el-tesoro-perdido.jpeg",
  "Mision Andina": null,
...
function renderMovies() {
  ...
  <button class="movie" data-id="${m.id}">
    ...
    <div class="movie-info">
      ...
      <p>★ ${m.clasificacion || 'Cine'} · ${m.duracionMinutos || '?'} min</p>
      <small>Elegir función</small>
    </div>
  </button>
  `).join('');

  document.querySelectorAll('.movie').forEach(b => 
    b.onclick = () => { 
      chooseMovie(Number(b.dataset.id)); 
      document.querySelector('#compra').scrollIntoView({behavior:'smooth'}); 
    }
  );
}
